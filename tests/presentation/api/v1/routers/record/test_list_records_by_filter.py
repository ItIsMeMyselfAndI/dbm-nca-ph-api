...
FILTER_VALUE = "Department of Health (DOH)"
TOTAL_DOH = 86
...
def test_list_records_by_filter_with_default_limit(client):
    response = client.get(
        f"/records/{FILTER_KEY.value}/{FILTER_VALUE}"
    )
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
...
def test_list_records_by_filter_with_limit_exceeding_total(client):
    response = client.get(f"/records/{FILTER_KEY.value}/{FILTER_VALUE}?limit=100")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
