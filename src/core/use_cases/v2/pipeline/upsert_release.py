...
class UpsertRelease:
    ...
    async def execute(self, release: Release) -> Tuple[Release, bool]:
        existing = await self.release_repository.get_release_by_id(release.id)
        if existing is not None:
            return (await self.release_repository.update_release(release.id, release), False)
        return (await self.release_repository.create_release(release), True)
