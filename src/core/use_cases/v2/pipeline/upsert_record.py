...
class UpsertRecord:
    ...
    async def execute(self, record: Record) -> Record:
        ...
        if existing_records:
            ...
            return await self.record_repository.update_record(existing.id, record)
        return await self.record_repository.create_record(record)
