...
class UpsertAllocation:
    ...
    async def execute(self, allocation: Allocation) -> Allocation:
        ...
        for existing in existing_allocations:
            if existing.agency == allocation.agency and existing.operating_unit == allocation.operating_unit:
                return await self.allocation_repository.update_allocation(existing.id, allocation)
        return await self.allocation_repository.create_allocation(allocation)
